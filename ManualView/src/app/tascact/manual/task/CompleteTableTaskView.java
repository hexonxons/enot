...
public class CompleteTableTaskView extends TaskView /*implements OnTouchListener*/{
	...
	{
		rect.set((int) (x*mColumnWidth) + mTableMargin + mLegendWidth, (int)(y*mRowHeight), 
				(int) (x*mColumnWidth + mColumnWidth) + mTableMargin + mLegendWidth, (int)(y*mRowHeight + mRowHeight));
	}
	...
	protected void onDraw(Canvas canvas)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			canvas.drawLine(mTableMargin, i*mRowHeight, 
					mTableWidth + mTableMargin, i*mRowHeight, dark);
			canvas.drawLine(mTableMargin, i*mRowHeight + 1, 
					mTableWidth + mTableMargin, i*mRowHeight + 1, highlight);
			
		}
		
		canvas.drawLine(mLegendWidth + mTableMargin, 0, 
				mLegendWidth + mTableMargin, mTableHeight, light);
		canvas.drawLine(mLegendWidth + mTableMargin + 1, 0,
				mLegendWidth + mTableMargin + 1, mTableHeight, highlight);
		
		for (int i = 0; i < mColNum + 1; i++)
		{
			canvas.drawLine(i*mColumnWidth + mLegendWidth + mTableMargin, 0,
					i*mColumnWidth + mLegendWidth + mTableMargin, mTableHeight, light);
			canvas.drawLine(i*mColumnWidth + mLegendWidth + mTableMargin + 1, 0, 
					i*mColumnWidth + mLegendWidth + mTableMargin + 1, mTableHeight, highlight);
		}
		...
		for (int i = 0; i < mColNum; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				...
				{
					foreground.setColor(getResources().getColor(R.color.table_edit));
		...
		for (int i = 0;  i < 3; i++)
		{
			canvas.drawText(InitialResources[InitialResources.length - 1][i], mTableMargin + x, 
						i*mRowHeight + y, foreground);
		}
		...
		for (int i = 0; i < KEYPAD_COUNT; i++)
		{
			...
			Rect r = new Rect(keyX, keyY, (int)(keyX + keyWidth), (int)(keyY + mRowHeight));
			canvas.drawRect(r, background);
			canvas.drawText(Integer.toString(i), keyX + x, 
					keyY + y, foreground);
			mKeypad[i] = r;
		}
		...
		for (int i = 0; i < mColNum; i++)	
		{
		 ...
		 {
			selected.setColor(getResources().getColor(R.color.table_incorrect));
			canvas.drawRect(toInvAfterCheck[i], selected);
			toInvAfterCheck[i] = null;
		 }
		 else if (toInvAfterCheck[i] != null && Checked[i])
		 {
			selected.setColor(getResources().getColor(R.color.table_correct));
			canvas.drawRect(toInvAfterCheck[i], selected);
			toInvAfterCheck[i] = null;
		 }
		}
		...
		else 
		{
			...
			for (int i = 0; i < KEYPAD_COUNT; i++)
				if (mKeypad[i].contains((int)X, (int)Y))
				{
					EDIT_MODE = true;
					selectedKey = i;
					mFilledCells[selectedX][selectedY] += Integer.toString(selectedKey);
					Rect toInv = new Rect(0, selRect.top, getWidth(), selRect.bottom);
					invalidate(toInv);
				}
			//
			...
			if (mKeypad[KEYPAD_COUNT + 1].contains((int)X, (int)Y))
			{
				...
				{
					mFilledCells[selectedX][selectedY] = mFilledCells[selectedX][selectedY].substring(0, mFilledCells[selectedX][selectedY].length() - 1);
				}
				Rect toInv = new Rect(0, selRect.top, getWidth(), selRect.bottom);
				invalidate(toInv);
			}
	...
	{
		return (y <= mTableHeight && x >= mTableMargin + mLegendWidth && x <= getWidth() - mTableMargin);
	}
